                            ...
                            <table class="table table-bordered table-hover" id="dataTable" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$contac->name}}</td>
                                        <td>{{$contac->email}}</td>
                                        <td>{{$contac->contact}}</td>
                                        <td>{{$contac->phone}}</td>
                                        <td>{{$contac->created_at}}</td>
                                        <td>{{$contac->updated_at}}</td>
                                        <td>
                                            <form action="{{ route('contact.destroy', $contac->id) }}" method="post">
                                                {{ method_field('DELETE') }}
                                                @csrf
                                                <button type="submit" class="btn btn-danger"
                                                    style="width:100px;">DELETE</button>
                                            </form>
                                            <a href="{{ route('contact.edit', $contac->id) }}">
